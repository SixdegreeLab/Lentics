...
async function main() {
  ...
  let migrateFileList = Array()
  fs.readdirSync('db/migrate').forEach(file => {
    if(file.startsWith('202')){
      migrateFileList.push(path.join('db/migrate', file))
    }
  });
  for (const [index, filename] of migrateFileList.entries()) {
    console.log(`${index+1}/${migrateFileList.length}, process ${filename}...`);
    let sql = fs.readFileSync(filename);
    await client.query(sql.toString());
  }
