export const RevenueSummarySql = `with paid_collect_detail as (
    select evt_tx_hash,
        evt_block_time,
...
consumption_summary as (
    select count(*) as consumer_count,
        sum(paid_amount_usd) as total_paid_amount_usd
    from user_consumption_summary
)

select consumer_count as "supporterCount",
    coalesce(trunc(total_paid_amount_usd::numeric, 2), 0) as "paidAmountUsd"
from consumption_summary;`

export const RevenueAmountByTokenSql = `with paid_collect_detail as (
    select evt_tx_hash,
        evt_block_time,
        date_trunc('day', to_timestamp(evt_block_time, 'YYYY/MM/DD HH24:MI:ss')) as block_date,
        collector,
...
token_revenue_summary as (
    select symbol,
        sum(paid_amount_usd) as total_paid_amount_usd
    from token_revenue_detail
    group by 1
)

select symbol,
    trunc(total_paid_amount_usd::numeric, 2) as "paidAmountUsd"
from token_revenue_summary;`
